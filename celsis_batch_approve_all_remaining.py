...
BG_PROGRESS_FILE = "approve_background_5_progress.json"

# 默认质控等待：1 到 2 分钟随机微调 (60 ~ 120 秒)，控制台按 Enter 可立即跳过
MIN_WAIT = 60
MAX_WAIT = 120

print("=" * 75, flush=True)
print("   EagleTrax Celsis 全量剩余样本自动审批引擎 (后台静默 + Enter 立即跳过)", flush=True)
print("   自动最小化到任务栏，绝不抢鼠标键盘焦点；倒计时期间按【Enter】立即处理下个样本！", flush=True)
print("=" * 75, flush=True)

# 1. 汇总所有历史已审批记录，避免重复审批
approved_set = set()
for log_fn in [PROGRESS_FILE, BG_PROGRESS_FILE, "approve_5_progress.json", "celsis_090926_approval_log.json"]:
...
try:
    for idx, sample_info in enumerate(remaining_queue, 1):
        ...
        # 实时写入两份进度文件
        for save_fn in [PROGRESS_FILE, BG_PROGRESS_FILE]:
    ...
    print("\n" + "=" * 75, flush=True)
    print("🏆🎉 全量 38 个白名单样本批量审批全部圆满完成！", flush=True)
