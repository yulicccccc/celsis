...
def extract_rows_from_page(page, page_num):
    ...
    for y, val in enumerate(proj):
        if val > 4000 and not in_line:
            ...
            in_line = False
            if y - start_y > 15: # at least 15 pixels tall
...
for idx in range(len(doc)):
    pnum = idx + 1
    print(f"Scanning Page {pnum}/{len(doc)}...", flush=True)
    pdata = extract_rows_from_page(doc[idx], pnum)
    all_pages_data.append(pdata)

with open("celsis_090926_rows_extracted.json", "w", encoding="utf-8") as f:
    json.dump(all_pages_data, f, indent=2)

print("Saved celsis_090926_rows_extracted.json successfully!")
