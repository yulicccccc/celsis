...
USERNAME = "qchen"
PIN = "1124"
AUDIT_FILE = "celsis_250926_final_audit_report.json"
PROGRESS_FILE = "celsis_250926_approval_progress.json"
...
print("=" * 75, flush=True)
print("   EagleTrax Celsis 25SEP26 全量样本自动审批引擎 (后台静默运行)", flush=True)
...
print(f"  • 物理熔断拦截:   {len(blocked)} 个 ({', '.join(b['Sample'] for b in blocked) if blocked else '无'}) -> 严禁触碰，已彻底隔离！")
...
try:
    for idx, sample_info in enumerate(remaining_queue, 1):
        ...
        if current_status.lower() != "data review":
            print(f"  ⛔ [SOP 状态拦截] 质控警报: 样本 {sample_id} 当前状态为 [{current_status}] (非 Data Review)！", flush=True)
